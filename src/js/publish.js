...
function lyxInit(data, uidToken) {
	...
	*/
	function Attachment(process, element) {
		this.process=process;
		this.element=element;
	}

	Attachment.prototype.changeState=function (state, args) {
		switch (state) {
			case "empty":
		...
		var uploader=new plupload.Uploader({
			...
			init: {
				...
				FileUploaded: function(uploader, file, info) {
		            ...
		            submitBody[type]=data["file_path"];
	...
	function check() {
		submitBody["title"]=$("#uwork-title").val();
		submitBody["main_content"]=editor.getValue();
		submitBody["audio_name"]=$("#uwork-audio-name").val();
		submitBody["video_name"]=$("#uwork-video-name").val();
		submitBody["video_code"]=$("#uwork-video-code").val();
		...
		if(submitBody["thumbnail"]=="") {
			$(".thumbnail-tip").show();
			result=false;
		}

		if(submitBody["main_content"]=="") {
		...
		if(choice.av=="audio") {
			...
			if(submitBody["audio_name"]=="") {
				$(".audio-name-tip").show();
				result=false;
			}
		}
		else {
			if(submitBody["video_name"]=="") {
				$(".video-name-tip").show();
				result=false;
			}

			if(choice.fc=="file") {
				if(submitBody["video"]==""&&data.method=="post") {
					$(".video-file-tip").show();
					result=false;
				}				
			}
			else {
				if(submitBody["video_code"]=="") {
					$(".video-code-tip").show();
					result=false;
				}				
			}			
